<script>
  ...
  const signOut = () => {
    // Insert code to clear user store
    navigate("/");
  };
</script>
...
>
  <div class="container px-4 mx-auto md:flex md:items-center">
    <div class="flex justify-between items-center">
      <Link to="/" class="font-bold text-xl text-gray-800">LOGO GOES HERE</Link>
      <button
    ...
    <!-- Nav bar notes: "Account" and "Trees" should only be visible if there is a user signed in (user store) -->
      ...
      >
      <!-- If user store has user, button text = "sign out", else "sign in" -->
    </div>
  </div>
</nav>
